...
fn filter_child_env(
    ...
    // inherit a socket path it holds no capability to reach: here that value is
    // the practical control, since AF_UNIX connect is unmediated unless
    // `af_unix_mediation` is enabled, and leaving it in place would also report
    // a misleading "Operation not permitted" rather than a truthful "no agent".
    //
...
mod tests {
    ...
    // ---------------------------------------------------------------------
    // local-socket credential availability.
    //
    // These mirror the macOS module's tests name for name. The env strip is
    // load-bearing here rather than defence-in-depth: Landlock does not mediate
    // AF_UNIX `connect` on a pathname socket unless `af_unix_mediation` is
    // enabled, so on this platform the environment variable is the practical
    // control over whether a command can reach an agent socket at all.
    // ---------------------------------------------------------------------

    /// Minimal state for the credential tests below.
    fn credential_test_state() -> ToolSandboxState {
        let runtime_dir = PathBuf::from("/tmp/nono-tool-sandbox-cred-test");
        let shim_dir = runtime_dir.join("shims");
        let shim = ShimIdentity {
            path: shim_dir.join("git"),
    ...
    fn credential_request_with_env(env: Vec<Vec<u8>>) -> ToolSandboxShimRequest {
    ...
    fn state_and_policy_with_socket(
        ...
        let mut state = credential_test_state();
    ...
    /// The variable is explicitly allow-listed, so this proves the strip beats
    ...
    fn filter_child_env_strips_allowed_env_var_for_unavailable_local_socket() -> Result<()> {
        ...
        let request = credential_request_with_env(vec![b"SSH_AUTH_SOCK=/host/agent.sock".to_vec()]);

        let env = filter_child_env(&state, &request, &policy, &Caller::Session { pid: 1 })?;

        assert!(
            !cred_env_contains_prefix(&env, b"SSH_AUTH_SOCK="),
    ...
    /// socket is unavailable — the strip runs after `apply_environment_set_vars`.
    #[test]
    fn filter_child_env_strips_set_var_for_unavailable_local_socket() -> Result<()> {
        ...
        let request = credential_request_with_env(Vec::new());
    ...
    /// bypassing `allow_vars`. The strip runs after it, so that route cannot
    /// re-admit the variable either.
    #[test]
    ...
    /// The strip is a separate pass running before every injection, so an
    ...
    fn filter_child_env_strip_does_not_clobber_proxy_credential_sharing_env_var() -> Result<()> {
        ...
        assert!(
            env.iter()
                .any(|entry| entry.as_slice() == b"SSH_AUTH_SOCK=/proxy/relay.sock"),
    ...
    /// filesystem read — the authority delta is a pure reduction.
    #[test]
    ...
    fn not_a_socket_is_fatal_per_command() {
        ...
        let request = credential_request_with_env(Vec::new());
        let env_err = filter_child_env(&state, &request, &policy, &Caller::Session { pid: 1 })
            .expect_err("a contradicted declaration must fail the command");
        assert!(
    ...
    /// like an absent one, instead of aborting every later invocation.
    #[test]
    fn socket_that_disappeared_mid_session_degrades_instead_of_aborting() -> Result<()> {
        ...
        let request = credential_request_with_env(vec![
    ...
    /// A live socket is still injected and still granted, unchanged.
    #[test]
    fn filter_child_env_injects_live_local_socket_credential() -> Result<()> {
        let tmp = tempfile::tempdir().map_err(|source| NonoError::ConfigWrite {
            path: PathBuf::from("/tmp"),
            source,
        })?;
        ...
        assert!(
            env.iter().any(|entry| {
                entry.as_slice() == format!("SSH_AUTH_SOCK={}", socket_path.display()).as_bytes()
            }),
            "a live socket must be injected: {env:?}"
        );
        assert!(
            !caps.unix_socket_capabilities().is_empty(),
            "a live socket must be granted"
        );
        Ok(())
    }
