...
If a referenced variable is unset, or the expanded path no longer resolves to a
socket at launch, the credential is *unavailable*: nono logs one warning naming
the variable, and commands that select the credential still run — without the
socket grant, and with the mapped `env_var` removed from their environment. This
covers both a host with no agent running and an agent that exits mid-session.
Grant the credential only where it is needed; a command that cannot work without
it should fail on its own terms rather than rely on nono to refuse the launch.

If the expanded path exists but is not a socket, that is a policy error rather
than an availability condition, and the individual command is denied.
...
| `local-socket credential unavailable; commands declaring it run without it` (warning) | The variable backing a local-socket credential is unset, or the path it expands to no longer resolves to a socket. Commands that select it still run, without the socket grant and without the mapped `env_var`. Start an agent if the command needs the credential. |
| `Tool Sandbox credential '<name>' is unavailable: <path> is not a socket` | A local-socket credential's path exists but is not a socket — a policy error, so the command is denied rather than run without the credential. |
